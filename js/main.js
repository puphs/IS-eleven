window.onload = function () {
    ...
    function wheel(event) {
        ...
        //console.log(delta);
        if (WINDOW.WIDTH - ZOOM_STEP > 0 &&
            WINDOW.HEIGHT - ZOOM_STEP > 0) {
            WINDOW.WIDTH += delta;
            WINDOW.HEIGHT += delta;
    ...
    function drawTFunction(m) {
        for (var t = -100; t <= 100; t++) {
            var arg = t / m;
            x = m * arg * Math.sin(Math.abs(arg));
            y = m / (Math.sin(Math.cos(arg)) + 6 * Math.cos(Math.sin(arg)));
            var windowX = ((t + 100) / 200) * WINDOW.width + WINDOW.LEFT;
            var windowY = ((t + 100) / 200) * WINDOW.HEIGHT + WINDOW.BOTTOM;
    ...
    function render() {
        ...
        // }
        drawFunction(f, 'blue', 3);
        drawFunction(g, 'red', 3);
    }
